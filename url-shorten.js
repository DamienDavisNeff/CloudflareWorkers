// REQUIRES A KV NAMESPACE
// in this code it's called "shortened-url"
// SLIGHTLY MODIFIED VERSION OF xyTom's Url-Shorten-Worker : https://github.com/xyTom/Url-Shorten-Worker
...
let response_header = {
  ...
  "Access-Control-Allow-Headers": "Content-Type", // Add this line to allow content-type header
};
...
async function save_url(URL){
    ...
    let is_exist=await shortened-url.get(random_key)
    console.log(is_exist)
    if (is_exist == null)
        return await shortened-url.put(random_key, URL),random_key
    else
        save_url(URL)
}
async function is_url_exist(url_sha512){
  let is_exist = await shortened-url.get(url_sha512)
  console.log(is_exist)
  if (is_exist == null) {
    return false
  }else{
...
async function handleRequest(request) {
    ...
    if (config.unique_link){
      ...
      if(url_key){
        ...
        if (typeof(stat) == "undefined"){
          console.log(await shortened-url.put(url_sha512,random_key))
        }
      }
    }else{
      stat,random_key=await save_url(req["url"])
  ...
  const value = await shortened-url.get(path);
  let location ;
